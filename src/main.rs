...
const INSTANCE_DISPLACEMENT: cgmath::Vector3<f32> = cgmath::Vector3::new(
    NUM_INSTANCES_PER_ROW as f32 - 1.0,
    -35.0,
    NUM_INSTANCES_PER_ROW as f32 - 1.0,
);
...
impl Net {
    fn new(context: &Context) -> Self {
        ...
        */
        ...
        let particles = (0..NUM_INSTANCES_PER_ROW * NUM_INSTANCES_PER_ROW)
            .map(|index| {
                ...
                let position = cgmath::Vector3 {
                    x: x as f32 * DIST_INTERVAL,
                    y: 0.0,
                    z: z as f32 * DIST_INTERVAL,
        ...
        */
        ...
        let sphere_pipeline = context.create_render_pipeline(
            // définit le pipeline pour le sphere
            "Sphere Pipeline",
        ...
        let sphere_vertex_buffer =
            context.create_buffer(&sphere_vertices, wgpu::BufferUsages::VERTEX);
        ...
        */
        ...
        /*
            Springs Pipeline
        */
        let compute_springs_pipeline = context
            .create_compute_pipeline("spring compute pipeline", include_str!("springs.wgsl"));
        ...
        for index in 0..particles.len() as i32 {
            ...
            for offset1 in [-1, 1] {
                for offset2 in [-1, 1] {
                    if col as i32 + offset1 >= 0
                        && col as i32 + offset1 < NUM_INSTANCES_PER_ROW as i32
                        && row as i32 + offset2 >= 0
                        && row as i32 + offset2 < NUM_INSTANCES_PER_ROW as i32
                    {
                        shear.push([
                            index,
                            index + (offset2 * NUM_INSTANCES_PER_ROW as i32) + offset1,
                        ]);
                    } else {
        ...
        let structural_index_buffer =
            context.create_buffer(structural.as_slice(), wgpu::BufferUsages::STORAGE);
        let shear_index_buffer =
            context.create_buffer(shear.as_slice(), wgpu::BufferUsages::STORAGE);
    ...
    fn update(&mut self, context: &Context, _delta_time: f32) {
...

        {
            ...
            compute_pass.dispatch_workgroups(
                (pow(NUM_INSTANCES_PER_ROW, 2) as f64 / 64.0).ceil() as u32,
                1,
                1,
            );
